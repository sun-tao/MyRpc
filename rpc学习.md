...
4. Netty服务端与客户端连接问题：
   ...
   - 长连接的维护问题：
     - 服务端存在问题：客户端下线了，但是服务端不知道，仍然维护着与客户端的连接，浪费系统资源。因此服务端应当加入超时检测机制`IdleStateHandler`，若超过一定时间没收到客户端的消息，则会断开此连接。为了防止服务端断开未下线，只是暂时不活跃的客户端连接，加入心跳机制来避免断开连接。
       ...
       *#TODO*：引入Netty心跳机制，客户端定时每隔15s发送一次心跳包，服务端收到心跳包确认客户端的活跃情况，防止服务端关闭活跃的客户端。[Done]
     
       *#TODO:* 心跳机制时引入请求ID和响应ID字段，这两个字段应该尽可能保证唯一性，可以采用`Atomic`的`getAndIncrement`获取ID。
     
     - 客户端存在问题：若是服务端下线了，客户端如何重新连接？
     
       客户端首次发起连接失败后，应该采用Dubbo的`failover`重试机制，向zk上的其他服务器发起Rpc请求，暂定2次重试。
       
       *#TODO*: Rpc重连机制。当客户端连接Netty服务端时，负载均衡选择的服务端无法连上的时候，可以触发重试机制，将排除该无法连上的服务端，尝试连接其他的服务端，如果无服务端可用，或者重试次数超过后仍然不行，则本次调用失败。[Done]
       
       *#TODO*: 补对重试机制的测试。测试通过[Done]
       
       *#TODO*: 重试机制代码封装，以便多个地方复用。
