...
4. Netty服务端与客户端连接问题：
   ...
   - 长连接的维护问题：
     ...
     - 客户端存在问题：若是服务端下线了，客户端如何重新连接？
       ...
       *#TODO*: Rpc重连机制。当客户端**连接**Netty服务端时，负载均衡选择的服务端无法连上的时候，可以触发重试机制，将排除该无法连上的服务端，尝试连接其他的服务端，如果无服务端可用，或者重试次数超过后仍然不行，则本次调用失败。[Done]
       
       *#TODO*: 补对重试机制的测试。测试通过[Done]
       
       *#TODO*: 服务端下线，客户端的重试机制。项目写法自带重连机制，如果服务端下线，则后续rpc请求的时候，该channel是不activate的，因此客户端会尝试重连，如果失败则会根据重试机制，换其他服务端进行连接。[Done]
...
6. *#TODO* Reactor线程模型：Reactor多线程模型和主从Reactor线程模型
